    ...
    try {
      ...
      if (Array.isArray(indices)) {
        for (const idx of indices) {
          if (idx?.name && idx?.pct != null) {
            if (idx.name === 'NIFTY 50') map['^NSEI'] = +idx.pct.toFixed(2)
            if (idx.name === 'NIFTY BANK') map['^NSEBANK'] = +idx.pct.toFixed(2)
          }
        }
      }
...
const FEATURES = [
  { icon: '📈', title: 'Live NSE Indices',     desc: 'Real-time NIFTY 50, BANK NIFTY, NIFTY IT and more via yfinance WebSocket streaming.' },
  { icon: '₹',   title: 'Indian NSE Stocks',   desc: 'Live NSE stock prices streamed via yfinance WebSocket. RELIANCE, TCS, INFY and more.' },
  { icon: '📊',  title: 'Market Indices',       desc: 'Live S&P 500, NASDAQ, DOW percentage changes updating in real time.' },
  { icon: '🤖',  title: 'AI Stock Analysis',    desc: 'Technical analysis with RSI, MACD, SMA, Bollinger Bands powered by Groq AI.' },
  { icon: '🔒',  title: 'Secure by Design',     desc: 'All API keys live server-side. Zero secrets reach your browser.' },
    ...
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: MONO, color: C.text }}>
      ...
      <div style={{ textAlign: 'center', padding: '80px 24px 60px', maxWidth: 800, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: 14, color: C.textSec, lineHeight: 1.8, marginBottom: 40, maxWidth: 560, margin: '0 auto 40px' }}>
          Live Indian NSE stocks, indices, and market data — all in one terminal.
      ...
      <div style={{ padding: '32px 24px', textAlign: 'center' }}>
        <div style={{ fontSize: 11, color: C.textDim, marginBottom: 8 }}>
          NSE INDICES + STOCKS: YFINANCE WEBSOCKET · US ETFS: FINNHUB · MF NAV: AMFI
        </div>
